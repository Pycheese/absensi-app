            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Perbarui jadwal sesi live shopping
                    </p>
                </div>
            ...
            <div class="bg-white rounded-[22px] border border-gray-100 shadow-sm p-6 max-w-4xl">
                ...
                <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST" class="space-y-5">
                        ...
                        <select name="brand_id"
                            ...
                            @foreach ($brands as $brand)
                                <option value="{{ $brand->id }}"
                                    {{ old('brand_id', $schedule->brand_id) == $brand->id ? 'selected' : '' }}>
                                    {{ $brand->name }}
                                </option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <input type="text"
                            name="session_name"
                            value="{{ old('session_name', $schedule->session_name) }}"
                    ...
                    <div>
                        ...
                        <input type="date"
                            name="session_date"
                            value="{{ old('session_date', \Carbon\Carbon::parse($schedule->session_date)->format('Y-m-d')) }}"
                            class="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#20BDE3] focus:bg-white"
                            required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text"
                            ...
                            placeholder="Contoh: Studio Jakarta"
                            class="w-full border border-gray-200 bg-gray-50 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#20BDE3] focus:bg-white"
                            required>
                    </div>
    <div>
            ...
            <div class="grid grid-cols-2 gap-3">
                ...
                @foreach ($users as $user)
                    <label
                        ...
                        <input type="checkbox" name="user_ids[]" value="{{ $user->id }}" {{ in_array($user->id, old('user_ids', $schedule->users->pluck('id')->toArray())) ? 'checked' : '' }} class="w-4 h-4 text-[#20BDE3]">
                ...
                @endforeach
                    ...
                    <div>
                        ...
                        <select name="is_active"
                            ...
                            <option value="1" {{ old('is_active', $schedule->is_active) == 1 ? 'selected' : '' }}>
                                Aktif
                            </option>

                            <option value="0" {{ old('is_active', $schedule->is_active) == 0 ? 'selected' : '' }}>
                                Tidak Aktif
                            </option>
